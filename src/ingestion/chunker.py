...
- Fit in the embedding model's context
...
class DocumentChunker:
    ...
    Strategy chosen to balance:
    - Semantic coherence (split on natural boundaries: paragraphs, sentences)
    - Embedding model context limits (most embedding models cap at 512 tokens)
    ...
    def __init__(
        ...
        # 1 token ≈ 4 chars (rough average for English)
        chars_per_token = 4

        self.splitter = RecursiveCharacterTextSplitter(
            chunk_size=chunk_size_tokens * chars_per_token,
            chunk_overlap=chunk_overlap_tokens * chars_per_token,
            # Order matters: splits on first separator that produces valid chunks
            separators=["\n\n", "\n", ". ", " ", ""],
            length_function=len,
        )
